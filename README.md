...
| 维度 | Claude Code | OpenClaw 类<br>（CLI 克隆） | n8n / Dify | **OpenAgentic** |
|---|---|---|---|---|
| 产品形态 | 开发者 CLI | 开发者 CLI | 工作流平台 | **平台**（CLI + API + Workflow + KB） |
| 中国可用 | ❌ 模型境外 | ⚠️ 模型自带 | ⚠️ 部分有限 | ✅ 接 DeepSeek / Qwen / Doubao / 私有 Ollama |
| 私有化部署 | ❌ | ✅ | ✅ | ✅ |
| 多租户 | ❌ | ❌ | ✅(SaaS) / ❌(自托管) | ✅ 行级隔离 + tenant context |
| AI 原生工作流 | ⚠️ | ❌ | ⚠️(加 LLM 节点) | ✅ DAG + 模板渲染 + 工具 + RAG 一体 |
| 行业 SOP 模板 | ❌ | ❌ | ⚠️(通用) | 🚧 通用原型 + 行业模板共建 |
| 多 provider | ❌(仅 Claude) | ⚠️ | ⚠️ | ✅ 20 个 |
...
| 节点连接器（HTTP/钉钉/企微/DB/邮件） | ❌ 无 | 当前只能调 LLM 和内置工具 |
...
1. **Phase 7 — Workflow 企业化**（主轴）：连接器 + 审批节点 + 模板库 + 可视化 + 版本+回滚。**别人不做、客户真要、你能做**的赛道。
...
| 任务 | 优先级 | 说明 | 验收 |
|---|---|---|---|
| **节点连接器** | **P0** | HTTP / 数据库（PG/MySQL/MongoDB）/ SMTP / 钉钉 / 企微 / Webhook / 文件系统 | 没连接器=玩具，必须先做 |
